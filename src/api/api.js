...
class API {
  ...
  // Get auth headers with CORS support
  getAuthHeaders() {
  ...
  // Handle response with CORS error handling
  async handleResponse(response) {
    if (!response.ok) {
      // Handle CORS errors specifically
      if (response.status === 0 || response.type === 'opaque') {
        throw new Error('CORS Error: Unable to access the API. Please check your network connection and API configuration.')
      }
  ...
  // Make request with retry logic and CORS handling
