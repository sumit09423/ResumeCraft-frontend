...
const ProfessionalTemplate = ({ data }) => {
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden max-w-4xl mx-auto">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 lg:p-8">
        <div className="text-center">
          <h1 className="text-2xl lg:text-4xl font-bold mb-2 break-words">
            {data.firstName} {data.lastName}
          </h1>
          <p className="text-lg lg:text-xl opacity-90 mb-6 break-words">
            {data.experiences[0]?.role || 'Professional'}
          </p>
          <div className="flex flex-col lg:flex-row justify-center lg:space-x-8 space-y-2 lg:space-y-0 text-sm">
            {data.email && (
      ...
      <div className="p-6 lg:p-8">
        ...
        {data.summary && (
          <div className="mb-8">
            ...
            <p className="text-gray-700 leading-relaxed text-base lg:text-lg break-words">{data.summary}</p>
          </div>
        )}
            ...
            {data.experiences.filter(exp => exp.company && exp.company.trim()).map((exp, index) => (
              <div key={index} className="mb-6 border-l-4 border-blue-200 pl-6">
                <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-2 gap-2">
                  ...
                  <span className="text-blue-600 font-medium text-sm flex-shrink-0">
                    {exp.startDate && new Date(exp.startDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                    {exp.endDate && ` - ${new Date(exp.endDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}`}
                    {exp.current && ' - Present'}
                  </span>
                </div>
                <h4 className="text-base lg:text-lg font-medium text-blue-600 mb-2 break-words">{exp.company}</h4>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              {data.skills.filter(skill => skill.name && skill.name.trim()).map((skill, index) => (
                <span
                  key={index}
                  className="inline-block bg-blue-100 text-blue-800 px-4 py-2 rounded-lg text-sm font-medium break-words border border-blue-200"
                >
                  {skill.name}
                </span>
              ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {data.hobbies.filter(hobby => hobby.name && hobby.name.trim()).map((hobby, index) => (
                  <span
                    key={index}
                    className="inline-block bg-blue-100 text-blue-800 px-3 py-1 rounded text-sm font-medium break-words"
                  >
                    {hobby.name}
                  </span>
                ))}
              ...
              <div className="space-y-3">
                {data.socialMedia.filter(social => social.platform && social.url).map((social, index) => (
                  <div key={index} className="flex items-center">
                    <div className="w-4 h-4 mr-3 text-blue-600 flex-shrink-0">
