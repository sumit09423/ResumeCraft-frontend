  ...
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden max-w-5xl mx-auto">
      <div className="flex flex-col lg:flex-row">
        {/* Left Sidebar */}
        <div className="w-full lg:w-1/3 bg-gray-900 text-white p-6 lg:p-8">
          {/* Profile Picture */}
          {data.profilePicture && (
            <div className="mb-6">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-300 text-base lg:text-lg break-words">
          ...
          {/* Contact Information */}
          <div className="mb-8">
            <h3 className="text-lg lg:text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Contact</h3>
            <div className="space-y-3">
              ...
              {data.mobileNumber && (
                <div className="flex items-start">
                  ...
                  <span className="text-sm break-words">{data.mobileNumber}</span>
                </div>
              )}
              {data.location && (
                <div className="flex items-start">
                  ...
                  <span className="text-sm break-words">{data.location}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Social Media */}
          {data.socialMedia && data.socialMedia.length > 0 && data.socialMedia.some(social => social.platform && social.url) && (
            <div className="mb-8">
              <h3 className="text-lg lg:text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Social Media</h3>
              <div className="space-y-3">
                {data.socialMedia.filter(social => social.platform && social.url).map((social, index) => (
                  <div key={index} className="flex items-center">
                    ...
                    <a
                      ...
                      className="text-blue-400 hover:text-blue-300 text-sm break-words"
                    >
                      {social.username}
                    </a>
                  </div>
          ...
          {/* Hobbies */}
          {data.hobbies && data.hobbies.length > 0 && data.hobbies.some(hobby => hobby.name && hobby.name.trim()) && (
            <div>
              <h3 className="text-lg lg:text-xl font-semibold mb-4 border-b border-gray-700 pb-2">Interests</h3>
              <div className="flex flex-wrap gap-2">
                {data.hobbies.filter(hobby => hobby.name && hobby.name.trim()).map((hobby, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gray-700 text-gray-200 px-3 py-1 rounded text-xs break-words"
                  >
                    {hobby.name}
                  </span>
                ))}
        ...
        <div className="w-full lg:w-2/3 p-6 lg:p-8">
          ...
          {data.summary && (
            <div className="mb-8">
              <h2 className="text-xl lg:text-2xl font-bold text-gray-800 mb-4 border-b-2 border-gray-300 pb-2">
                Professional Summary
              </h2>
              <p className="text-gray-700 leading-relaxed text-sm lg:text-base break-words">{data.summary}</p>
            </div>
          )}

          {/* Work Experience */}
          {data.experiences && data.experiences.length > 0 && data.experiences.some(exp => exp.company && exp.company.trim()) && (
            <div className="mb-8">
              <h2 className="text-xl lg:text-2xl font-bold text-gray-800 mb-4 border-b-2 border-gray-300 pb-2">
                Work Experience
              </h2>
              {data.experiences.filter(exp => exp.company && exp.company.trim()).map((exp, index) => (
                <div key={index} className="mb-6">
                  ...
                  <h4 className="text-base lg:text-lg font-medium text-gray-600 mb-2 break-words">{exp.company}</h4>
                  ...
                  {exp.achievements && exp.achievements.length > 0 && (
                    <ul className="list-disc list-inside text-gray-600 space-y-1">
                      {exp.achievements.map((achievement, idx) => (
                        <li key={idx} className="text-sm break-words">{achievement}</li>
                      ))}
                    </ul>
                  )}
                </div>
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-2">
                {data.skills.filter(skill => skill.name && skill.name.trim()).map((skill, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gray-200 text-gray-800 px-3 py-1 rounded text-sm font-medium break-words"
                  >
                    {skill.name}
                  </span>
                ))}
          ...
          {/* Projects */}
          {data.projects && data.projects.length > 0 && data.projects.some(project => project.name && project.name.trim()) && (
            <div>
              <h2 className="text-xl lg:text-2xl font-bold text-gray-800 mb-4 border-b-2 border-gray-300 pb-2">
                Projects
              </h2>
              {data.projects.filter(project => project.name && project.name.trim()).map((project, index) => (
                <div key={index} className="mb-6">
                  <h3 className="text-lg font-semibold text-gray-800 mb-2 break-words">{project.name}</h3>
                  <p className="text-gray-700 mb-2 leading-relaxed text-sm lg:text-base break-words">{project.description}</p>
                  {project.technologies && project.technologies.length > 0 && (
                    <p className="text-gray-600 mb-2 text-sm lg:text-base break-words">
                  ...
                  <div className="flex flex-wrap gap-4 mb-3">
                    {project.githubUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-blue-600 hover:text-blue-700 text-sm break-words"
                      >
                        GitHub
                      </a>
                    )}
